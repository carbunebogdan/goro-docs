...
Results are not stored on this surface either. Rows arrive in the `run_tool`
response, or in the `get_run` poll that finds the run finished. Have the agent
write down anything it will need later.
