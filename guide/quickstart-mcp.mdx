...
description: "Add Goro to Claude, Cursor or any MCP client as a connector, and your agent gets the whole catalog as eight tools."
---

Goro runs a remote MCP server. Add one URL to your client and the agent gets
eight tools that reach the entire catalog, on the same balance and the same
...
<Note>
  Connecting does not mean the workspace can run tools yet. `discover_tools`,
  `inspect_tool` and `wallet_balance` work right away. `run_tool` and
  `create_upload` need an active plan, Build or Scale, and answer
  `subscription_required` until the workspace has one. Pick a plan at
  [app.usegoro.ai/billing](https://app.usegoro.ai/billing).
</Note>
...
| `create_upload` | `tools:read` | Before `run_tool`, whenever the tool needs a file. Needs an active plan, even though it spends nothing |
| `run_tool` | `tools:run` | To actually execute a tool. Needs an active plan. This spends money |
...
That is the complete list. There is no per-tool MCP surface: the 77 catalog
...
`wallet_balance` returns the available balance, the workspace's `plan`, the
unexpired included credit, `renews_at` and a `top_up_url`. The URL and, on a
plan, the renewal date are what an agent should surface to a human when funds
run out.
...
| `subscription_required` | Stop. The workspace has no active plan. Tell the human to pick one at the `billing_url` in the body |
| `insufficient_funds` | Stop. Tell the human the amount, the `top_up_url` and, if `renews_at` is set, when included credit returns |
...
<Card title="Full API reference" icon="code" href="/api/overview">
  The endpoints behind the eight tools.
</Card>
