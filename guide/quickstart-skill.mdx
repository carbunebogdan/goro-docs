...
Save the following as a skill file, a system prompt, or whatever your framework
calls its instruction file. It is the whole contract.
...
4. POLL, only after a 202.
   ...
   Safe to repeat. The poll that finds the run finished carries the rows in
   `output`. Keep them: Goro does not store results, so a later poll of the
   same run returns status and cost with `output: null`.
