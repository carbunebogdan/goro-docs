...
Create a workspace at [app.usegoro.ai](https://app.usegoro.ai), pick a plan
under **Billing** (Build or Scale), and generate an API key. Running a tool
needs an active plan: without one every call to `run` answers `402
subscription_required`, so pick a plan before you wire up the agent.
...
Goro is a tool gateway. One key, one balance, pay per call. You never
...
- On 402 `subscription_required`, the workspace has no active plan. Stop and
  tell your human to pick one at the `billing_url` in the body. Do not retry.
- On 402 `insufficient_funds`, stop and tell your human the amount and the
  `top_up_url` in the body. If the body's `renews_at` is set, say that the
  plan's included credit comes back on that date. Do not keep retrying.
