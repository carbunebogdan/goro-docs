...
<Note>
  ...
  rows untouched. `input_schema` is that tool's own schema, not a translation,
  so any field the tool accepts works even when the schema does not name it.
  The schema is what Goro validates against, so undeclared fields pass through
