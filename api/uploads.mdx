...
Creating an upload still needs an active plan, the same as running a tool:
without one this answers `402 subscription_required`, even though the call
itself is free. See [Pricing](/pricing#plan-rules).
...
| 402 | `subscription_required` | The workspace has no active plan. Carries `billing_url` and `top_up_url` |
