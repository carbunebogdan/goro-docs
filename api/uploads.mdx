...
```json
{
  ...
  "next": {
    "upload": "curl -X PUT --data-binary @<file> -H \"Content-Type: image/jpeg\" \"<upload_url>\"",
...
| `upload_url` | string | Where to PUT the file. Signed, single use, and it addresses the storage bucket rather than Goro |
...
| --- | --- | --- |
| `image_handle` | Any file you hold locally or privately. This is the road most callers should take | 10 MB |
| `image_urls` | The image is already on a public https link, including the output URL of an image endpoint | 10 MB |
| `image_base64` | The image is genuinely small, or you have no shell to run the PUT with | 3 MB |

`image_base64` is not going away. It is the right answer for a small file and
for a caller that cannot make a second request. It is the wrong answer for a
photograph, and the failure it produces is silent enough to be worth naming: a
truncated base64 string decodes to fewer, wrong bytes rather than to an error.
...
`content_type`, returns the same handle and URL, and includes the exact `curl`
to run next. It needs the `tools:read` scope rather than `tools:run`, because it
spends nothing.

The instruction for an agent is short enough to repeat: never read a file into
the conversation in order to send it. Call `create_upload`, PUT the file with
your own shell, then pass the handle to `run_tool`. Do not resize the image to
make it fit, because fitting is not the problem the size was causing.
