...
Running a tool needs an active plan, Build or Scale. Without one this answers
`402 subscription_required` before anything is held, and nothing is charged.
See [Pricing](/pricing#plan-rules).
...
```json
{
  ...
  "price": {
    ...
    "per_unit_usd": "$0.0075",
    "unit_label": "page",
    "summary": "$0.0075 per page, billed on actual usage (minimum $0.0020 per call), up to $1.00 per call"
  }
}
```
...
```json
{
  ...
  "price": { "type": "usage_based", "billed_on_actual_usage": true, "summary": "..." }
}
```
...
- `usage_based`, which is most of the catalog: worked out from what the call
  actually did, floored at `min_micro` and capped at both `max_micro` and the
  hold. Whatever is left of the hold is released. A run that returned nothing,
  or that cost nothing to serve, is billed nothing and comes back `free`.
- `per_call`: `base_micro`, every time. The quote and the charge are the same
  number, so nothing is released.
- `per_call_variant`, the nano banana image tools: the price of the option you
  asked for, picked from `variants` by the input named in `selector` (for those
  two, `resolution`). Still flat and still exact, so the quote and the charge
  are again the same number and nothing is released. Omit the field and you get
  `default_variant`'s price.
...
<Note>
  On a `usage_based` run Goro waits for the provider's own billing to settle
...
| 402 | `subscription_required` | The workspace has no active plan. Carries `billing_url` and `top_up_url`. Nothing is held |
| 402 | `insufficient_funds` | The wallet cannot cover the hold. Carries `top_up_url`, `required_micro`, `plan` and `renews_at` |
