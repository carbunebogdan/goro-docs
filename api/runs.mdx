...
<Warning>
  ...
  the list endpoint reads only what is stored. Use `GET /v1/runs/{id}` while a
  run is still in flight to receive its rows.
</Warning>
...
Goro never stores results, so a poll of a run that was already terminal before
you asked returns its status, counts and cost with `output: null`. A re-read of
the provider's own copy is best effort and providers age their datasets out, so
`output: null` on an older run is a normal outcome rather than an error.

If you need results you can come back to, write them somewhere yourself when
