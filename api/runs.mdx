...
Goro never stores results, so this endpoint re-reads them from the provider's
own copy. That works whether the run is still finishing or already settled: a
poll that arrives after the run completed still returns its rows.

The re-read is best effort. Providers age their datasets out, so `output: null`
on an older run is a normal outcome rather than an error, and it is the reason
to write results somewhere yourself if you need them long term.
