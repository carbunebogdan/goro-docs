...
- Once a run is already terminal in Goro's records, a later `GET
  /v1/runs/{id}` returns its status, counts and cost with `output: null`. The
  rows are not stored, so there is nothing to return.
