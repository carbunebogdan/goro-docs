...
<Note>
  Signing up needs no plan, and neither does discovery: `discover`, `inspect`,
  listing and fetching runs, and reading the wallet all work on a workspace
  with no plan. `POST /v1/run` and `POST /v1/uploads` need an active plan,
  Build or Scale; without one they answer `402 subscription_required`. See
  [Errors](#errors) below and [Pricing](/pricing#plan-rules).
</Note>
...
| 402 | `subscription_required` | `billing_url`, `top_up_url` | The workspace has no active plan. `POST /v1/run` and `POST /v1/uploads` need one; nothing is held or charged |
| 402 | `insufficient_funds` | `top_up_url`, `required_micro`, `available_micro`, `plan`, `renews_at` | The wallet cannot cover the hold. On a plan, `renews_at` is when included credit returns and the message names that date |
