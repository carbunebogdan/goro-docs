...
## Run results are not stored

Goro records what a run cost, how many rows it produced and when it ran. It
never writes the rows themselves to the database: `runs.output` is null in
storage for every run, always.
