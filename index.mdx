...
The MCP server exposes five tools (`discover_tools`, `inspect_tool`,
`run_tool`, `get_run`, `wallet_balance`), each a thin wrapper over the same
code the HTTP endpoints call.
...
It has a consequence worth planning for. A synchronous call returns its rows
inline, and an asynchronous poll re-reads them from the provider that produced
them. Providers age their datasets out, so a run's rows can stop being
available even though the run's status and cost stay readable forever. Keep the
rows from the response that hands them to you.
