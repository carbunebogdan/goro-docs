...
Generate an original image from a text prompt, or EDIT images you supply, using Google Nano Banana Pro at 2K resolution. Send image_handles, image_urls or image_base64 to edit, restyle or combine up to 8 reference images; send none and it generates from the prompt alone. Editing costs exactly what generating costs and the extra references are free. The most capable image model in the catalog, and priced accordingly. Returns the URL of the generated image. Those URLs are temporary: Goro holds a run's results for 24 hours and nothing guarantees the file outlives that, so collect what you need within 24 hours.
...
**$0.18 per call at `2K`.** A flat price, and the only thing that changes it is `resolution`: `2K` costs **$0.18**, `4K` costs **$0.24**.
...
| `resolution` | `2K`, `4K` |  | Output resolution, and the only thing that changes the price: $0.18 at 2K, $0.24 at 4K. Defaults to 2K. 1K is not offered because this model costs the same at 1K as at 2K, so it would be a smaller image for the same money. Defaults to `"2K"`. |
