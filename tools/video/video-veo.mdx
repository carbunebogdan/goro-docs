...
Generate an 8 second video with sound using Google Veo 3.1 at its Quality tier in 1080p, from a text prompt or from a still image you supply. Returns the URL of the generated MP4. That URL is temporary: Goro holds a run's results for 24 hours and nothing guarantees the file outlives that, so collect what you need within 24 hours.
...
**$2.55 per call.** A flat price. Every call that succeeds costs exactly this, whatever it returns, and you know the number before you make the call.
