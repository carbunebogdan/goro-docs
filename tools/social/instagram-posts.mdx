...
**$0.0150 per post.** That is the rate you pay for each post the call returns, so the total depends on how many it produces.
...
| `dataDetailLevel` | `basicData`, `detailedData` |  | detailedData is billed extra at $0.008 per post Defaults to `"basicData"`. |
